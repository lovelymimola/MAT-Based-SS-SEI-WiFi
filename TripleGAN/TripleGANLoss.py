...
class Loss_of_Classifier(nn.Module):
    ...
    def forward(self,logits_all, target_all):
        ...
        ce_l = cross_entropy_loss(logits_all[0:len(logits_all)//2],target_all[0:len(logits_all)//2])
        ce_g = cross_entropy_loss(logits_all[len(logits_all)//2:len(logits_all)],target_all[len(logits_all)//2:len(logits_all)])
        loss_c = ce_l + ce_g
...
def z_obeys_nakagami(data_label, bs_data_label,m=1):
    ...
    '''
    z = torch.zeros((bs_data_label,6000))
    for i in range(bs_data_label):
        p = torch.mean(data_label[i,:])
        z[i,:] = torch.tensor(nakagami.rvs(m, loc = 0, scale = abs(p), size = 6000))
