...
class Config:
    def __init__(
        ...
        ft: int = 62,
        n_classes: int = 16,
        save_path: str = 'model_weight/SSRCNN_n_classes_16_10label_90unlabel_rand30.pth',
        device_num: int = 0,
        rand_num: int = 30,
        ):
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_SSRCNN_n_classes_16_10label_rand30")
    device = torch.device("cuda:"+str(conf.device_num))

    RANDOM_SEED = 300  # any random number
    set_seed(RANDOM_SEED)

    X_train_label, X_train_unlabel, X_val, Y_train_label, Y_train_unlabel, Y_val = TrainDataset(conf.ft, conf.rand_num)
