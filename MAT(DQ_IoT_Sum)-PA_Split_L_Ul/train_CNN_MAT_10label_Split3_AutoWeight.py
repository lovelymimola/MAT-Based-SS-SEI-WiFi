...
def train(model, loss, loss_ProxyAnchor, train_dataloader, VAT_dataloader, optimizer, awl, epoch, writer, device_num):
    ...
    print(
        'Train Epoch: {} \tClassifier_Loss (Label): {:.6f}, Classifier_Loss (Unlabel): {:.6f}, Proxy Anchor Loss (Label): {:.6f}, Proxy Anchor Loss (Unlabel): {:.6f}, VAT Loss (Unlabel): {:.6f}, Combined_Loss :{:.6f}, Combined_Loss :{:.6f}, Accuracy: {}/{} ({:0f}%)\n'.format(
        epoch,
...
class Config:
    def __init__(
        ...
        n_classes: int = 16,
        ft: int = 62,
        save_path: str = 'model_weight/CNN_MAT1_n_classes_16_10label_90unlabel_rand30_autoweight100.pth',
        device_num: int = 0,
        rand_num: int = 30,
        ):
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_CNN_MAT1_n_classes_16_10label_90unlabel_rand30_Split3_autoweight100")
    ...
    loss_ProxyAnchor = Proxy_Anchor(nb_classes = conf.n_classes, sz_embed = 1024, mrg = 0.1, alpha = 32)
    if torch.cuda.is_available():
