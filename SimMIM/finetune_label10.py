...
class Config:
    def __init__(
        ...
        encoder_original_save_path: str = 'model_weight/SimMIM_encoder_mask05_n_classes_16.pth',
        encoder_save_path: str = 'model_weight/SimMIM_encoder_mask05_n_classes_16_label10.pth',
        classifier_save_path: str = 'model_weight/SimMIM_classifier_mask05_n_classes_16_label10.pth',
        device_num: int = 0,
        n_classes: int = 10,
        rand_num: int = 30,
        ):
...
def main():
    conf = Config()
    writer = SummaryWriter("logs_SimMIM_finetune_mask05_n_classes_16_label10")
    device = torch.device("cuda:" + str(conf.device_num))

    ft = 62
    X_train_label, X_train_unlabel, X_val, Y_train_label, Y_train_unlabel, Y_val = TrainDataset(ft, conf.rand_num)

    train_dataset_label = TensorDataset(torch.Tensor(X_train_label), torch.Tensor(Y_train_label))
    ...
    X_test, value_Y_test = TestDataset(ft)
